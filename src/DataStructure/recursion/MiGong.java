package DataStructure.recursion;

public class MiGong {
    public static void main(String[] args) {
        int[][] map = new int[8][7];
        //1代表墙
        for (int i = 0; i < 7; i++) {
            map[0][i] = 1;
            map[7][i] = 1;
        }
        for (int i = 0; i < 8; i++) {
            map[i][0] = 1;
            map[i][6] = 1;
        }
        map[3][1] = 1;
        map[3][2] = 1;
//        map[2][2] = 1;
//        map[1][2] = 1;

        //打印迷宫
        System.out.println("地图的情况为：");
        for (int i = 0; i < map.length; i++) {
            for (int j = 0; j < map[0].length; j++) {
                System.out.print(map[i][j] + " ");
            }
            System.out.println();
        }

//        boolean flag = setWay(map, 1, 1);
        boolean flag = setWay2(map, 1, 1);
        if (flag){
            System.out.println("能找到路");
        }else {
            System.out.println("找不到路");
        }
        //输出新的地图, 小球走过，并标识过的递归
        System.out.println("小球走过并标识过的地图为：");
        for (int i = 0; i < map.length; i++) {
            for (int j = 0; j < map[0].length; j++) {
                System.out.print(map[i][j] + " ");
            }
            System.out.println();
        }
    }

    //使用递归回溯来给小球找路
    //说明
    //1. map表示地图
    //2. i,j表示从地图的哪个位置开始出发
    //3. 若小球能到map[6][5]位置，则说明通路找到
    //4. 约定：当map[i][j]为0表示该点没有走过 1表示墙；2表示通路；3表示走过的点，但是走不通
    //5. 走迷宫时，需要确定策略：下->右->上->左，如果该点走不通，再回溯

    /**
     * @param map 表示地图
     * @param i   表示开始位置所在行
     * @param j   表示开始位置所在列
     * @return
     */
    public static boolean setWay(int[][] map, int i, int j) {
        if (map[6][5] == 2) {
            return true;
        } else {
            if (map[i][j] == 0) {
                map[i][j] = 2;
                //策略：下->右->上->左
                if (setWay(map, i + 1, j)) {   //向下
                    return true;
                } else if (setWay(map, i, j + 1)) {   //向右
                    return true;
                } else if (setWay(map, i - 1, j)) {   //向上
                    return true;
                } else if (setWay(map, i, j - 1)){  //向左
                    return true;
                }else {
                    //如果不能走
                    map[i][j] = 3;
                    return false;
                }
            }else {
                return false;
            }
        }

    }

    //修改策略为：上=>右=>下=>左
    public static boolean setWay2(int[][] map, int i, int j) {
        if (map[6][5] == 2) {
            return true;
        } else {
            if (map[i][j] == 0) {
                map[i][j] = 2;
                //策略：下->右->上->左
                if (setWay2(map, i - 1, j)) {   //向上
                    return true;
                } else if (setWay2(map, i, j + 1)) {   //向右
                    return true;
                } else if (setWay2(map, i + 1, j)) {   //向下
                    return true;
                } else if (setWay2(map, i, j - 1)){  //向左
                    return true;
                }else {
                    //如果不能走
                    map[i][j] = 3;
                    return false;
                }
            }else {
                return false;
            }
        }

    }
}
